-- AutoWalkSystem.lua
-- Place this file's contents into a LocalScript under StarterPlayer > StarterPlayerScripts.
-- This is a self-contained version: includes recording + local playback + simple "Auto Walk System" GUI.
-- Keys: F = toggle Record, P = Play (Local), L = Clear
...
local function rebuildSavedList()
	...
	for i, name in ipairs(savedItems) do
		...
		btn.MouseButton1Click:Connect(function()
			info("Selected checkpoint: "..tostring(name).." (load not persisted in this simple version)")
		end)
...
saveBtn.MouseButton1Click:Connect(function()
	...
	rebuildSavedList()
	info("Saved checkpoint name locally: "..name.." (no DataStore in this simple .lua)")
end)
...
info("Auto Walk System (.lua) ready. F=Record, P=Play, L=Clear.")
